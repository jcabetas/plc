...
*/
/*
class SDM630CT : public bloque
{
    //SDM630CT MedidorFlexo MB1 2
protected:
    uint16_t numModBus;
...
public:
 ...
 - Crear IO modbus, con MODBUS (Numero de modbus, interfaceModbus, baudios)
    MODBUS 1 MB1 9600
 - Crear dispositivo que use el MODBUS, p.e. SDM630CT con direccion=2
    SDM630CT MedidorFlexo 2 (nombreDispo, direccion)
 ...
 - Thread a nivel de Modbus.
   ...
   * usoBus(): debe mirar si necesita actualizar datos, y si procede usar el bus.
   ...
   El sdm630CT debe tener punteros a las medidas
...
sdm630ct::sdm630ct(BaseSequentialStream *tty,uint8_t numPar, char *pars[], uint8_t *hayError)
{
    if (numPar!=3)
    {
        nextion::enviaLog(tty,"#parametros incorrecto SDM630CT");
        *hayError = 1;
        return;
    }
...
uint16_t sdm630ct::diIdNombre(void)
{
    return idNombreDisp;
}
...
uint8_t sdm630ct::attachMedidas(medida *med, uint8_t numPar, char *pars[], uint8_t *hayError)
{
    // pars: P
    ...
    {
        if (!strcasecmp(descMedida630CT[tip],pars[0]))
        {
            tipo[numMedidas] = tip;
...
void sdm630ct::leer(float *valor, uint16_t addressReg, int16_t *error)
{
    float valor2;
    ...
    for (uint8_t m=1;m<=numMedidas;m++)
    {
        ...
        if (med->necesitaUpdate())
        {
            ...
            if (error!=0)
            {
                ...
                {
                    erroresSeguidos++;
                    // si ha fallado 5 veces, desechala
                    if (erroresSeguidos>4)
                    {
                        med->set(&valor, 1); // si hay error, vale 0.0f
                        med->setValidez(0);
                    }
                    return 2; // si no es muy grave, considera solo como no leido
                }
            }
            else
                erroresSeguidos = 0;
            med->set(&valor, 0); // el set valida medida
        }
    }
    if (heEnviado)
        return 0;
...
void sdm630ct::changeID(uint8_t oldId, uint8_t newId, int16_t *error)
{
    float newIdFloat;
    uint16_t bytesReceived;
    uint16_t msgCRC, rxCRC;
